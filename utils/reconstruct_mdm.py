...
bonds2 = {
    "C": {"C": 134, "N": 129, "O": 120, "S": 160, "Se": 215},
    "N": {"C": 129, "N": 125, "O": 121},
...
def make_mol_openbabel(positions, atom_types, dataset_info):
    ...
    """
    atom_decoder = dataset_info["atom_decoder"]
    atom_types_water = [atom_decoder[x] for x in atom_types]
    positions_water = copy.deepcopy(positions)

    # Remove water atoms from atom_types and positions
    non_water_indices = [
        i for i, atom in enumerate(atom_types_water) if atom != "water"
    ]
    water_indices = [i for i, atom in enumerate(atom_types_water) if atom == "water"]
    atom_types = [atom_types_water[i] for i in non_water_indices]
    positions = positions_water[non_water_indices]
    positions_only_water = positions_water[water_indices]
    if any(atom == "water" for atom in atom_types_water):
        print("Has water!")
    ...
    return mol, positions_only_water

...
def make_obmol(xyz, atomic_numbers):
    ...
    mol.BeginModify()
    atoms = []
    for i, (xyz, t) in enumerate(zip(xyz, atomic_numbers)):
        x, y, z = xyz
        atom = mol.NewAtom()
        ...
        if isinstance(t, (int, np.integer)):
            ...
            atomic_num = int(t.item())
            
        # Handle crystal water (marked as 1000)
        if atomic_num == 1000:
            # Treat crystal water as oxygen atom
            atomic_num = 8  # Oxygen
            # 我们可以使用其他方法来标记结晶水，比如：
            atom.SetType("O.w")  # 使用类型标记
            # 或者
            # atom.SetFormalCharge(0)  # 确保没有形式电荷
            # atom.SetHyb(3)  # sp3杂化
        elif atomic_num <= 0 or atomic_num > 118:
            raise MolReconsError(f"Invalid atomic number {atomic_num} at index {i}")

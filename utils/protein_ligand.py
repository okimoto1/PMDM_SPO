...
class PDBProtein(object):
    ...
    def _enum_formatted_atom_lines(self):
        for line in self.block.splitlines():
            if line[0:6].strip() == "ATOM" or (os.environ.get("WATER") == "protein" and line[0:6].strip() == "HETATM" and "HOH" in line):
                element_symb = (
                    line[76:78].strip().capitalize()
                )  # C or N or O or other elements
    ...
    def _parse(self):
        ...
        for atom in self._enum_formatted_atom_lines():
            if atom["type"] == "HEADER": # W3 file has no header
            ...
            self.is_backbone.append(atom["atom_name"] in self.BACKBONE_NAMES and atom["res_name"] != "HOH")
    ...
    def to_dict_atom_cutoff(self, ligand_pos, cutoff):
        ...
        for residue in self.residues:
            if (
                ((residue["center_of_mass"] - ligand_pos) ** 2).sum(-1) ** 0.5
            ).min() < cutoff:
                for atom_idx in residue["atoms"]:
...
def parse_water_pdb(path):
    # 读取pdb文件，找到包含HOH的行，返回HOH的坐标
    ...
    for atom_line in map(lambda x: x.split(), lines[3:]):

        if atom_line[0] == "END":
            break

        if len(atom_line) < 4 or atom_line[3] != "HOH":
            continue

        if atom_line[7].startswith("1.00"):
            index = atom_line[5].find("-", 1)
            if index != -1:
                x = atom_line[5][:index]
                y = atom_line[5][index:]
                z = atom_line[6]
            else:
                index = atom_line[6].find("-", 1)
                x = atom_line[5]
                y = atom_line[6][:index]
                z = atom_line[6][index:]

            print(f"{atom_line}: {x}, {y}, {z}")
        else:
            x, y, z = map(float, atom_line[5:8])
        pos.append([x, y, z])

    return np.array(pos, dtype=np.float32)


def parse_water_sdf(path):
    # 读取sdf文件，找到包含HOH的行，返回HOH的坐标

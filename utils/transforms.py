...
class FeaturizeProteinAtom(object):
    ...
    def __init__(self, dataset="crossdock", pocket=False):
        super().__init__()
        if dataset == "crossdock" or "pdbind" or "crossdock_pdbind":
            if not pocket:
                self.atomic_numbers = torch.LongTensor(
                    [1, 6, 7, 8, 16, 34]
                )  # crossdock H, C, N, O, S, Se
            else:
                self.atomic_numbers = torch.LongTensor(
                    [1, 6, 7, 8, 9, 15, 16, 17, 34, 119]
                )  # crossdock pocket
        # if dataset == 'pdbind':
...
class FeaturizeLigandAtom(object):
    ...
    def __init__(self, dataset="crossdock", pocket=False, pdbind_filter="original"):
        ...
        if dataset == "crossdock" or "pdbind" or "crossdock_pdbind":
            if not pocket:
                ...
                atom_list = [1, 6, 7, 8, 9, 15, 16, 17, 34, 119]
                if os.environ.get("WATER") == "ligand":
                    atom_list.append(1000)  # 1000 for water
                self.atomic_numbers = torch.LongTensor(atom_list)
        # if dataset == 'pdbind':
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        data.ligand_atom_feature = (
            element  # .float()+torch.randn_like(element.float()).to(element.device)
        )
        data.ligand_atom_feature_charge = torch.cat(
            [data.ligand_atom_feature_full, data.ligand_element.unsqueeze(1)], dim=1
        )
...
class LigandRandomMask(object):
    ...
    def __init__(
        self, min_ratio=0.0, max_ratio=1.2, min_num_masked=1, min_num_unmasked=0
    ):
        super().__init__()
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        data.ligand_masked_feature = data.ligand_atom_feature[
            masked_idx
        ]  # For Prediction
        ...
        data.ligand_frontier = (
            data.ligand_context_num_neighbors < data.ligand_num_neighbors[context_idx]
        )
...
class LigandMixedMask(object):
    ...
    def __init__(
        ...
        self.t = [
            ...
            LigandBFSMask(
                min_ratio, max_ratio, min_num_masked, min_num_unmasked, inverse=False
            ),
            LigandBFSMask(
                min_ratio, max_ratio, min_num_masked, min_num_unmasked, inverse=True
            ),
        ]
...
class ContrastiveSample(object):
    ...
    def __call__(self, data: ProteinLigandData):
        ...
        real_sample_idx = np.random.choice(
            np.arange(pos_real_mode.size(0)), size=self.num_real
        )
        ...
        fake_sample_idx = np.random.choice(
            np.arange(pos_fake_mode.size(0)), size=self.num_fake
        )
...
class AddHigherOrderEdges(object):
    ...
    def __call__(self, data: Data):
        ...
        type_mat = to_dense_adj(
            data.ligand_bond_index, edge_attr=data.ligand_bond_type
        ).squeeze(
            0
        )  # (N, N)
        ...
        data.ligand_edge_index, data.ligand_edge_type = coalesce(
...
class GetConformerPos(object):
    ...
    def __call__(self, data):
        ...
        conf_pos = torch.tensor(
            rdmol.GetConformer(0).GetPositions(), dtype=torch.float32
        )
        data.rdmol = rdmol
    ...
    def __call__(self, data):
        ...
        if not self.only_prot:
            if self.cutoff is None:
                ...
                ligand_adj = radius_graph(
                    data.ligand_pos, self.cutoff, batch=None, loop=False
                )
        ...
        protein_bond_type = (
            torch.ones(protein_adj.size(1), dtype=torch.long) * 4
        )  # define the protien edge type as 2
...
class RadiusEdge(object):
    ...
    def __call__(self, data):
        ...
        data.ligand_bond_index = radius_graph(
            data.ligand_pos, 2, batch=None, loop=False
        )
        # print(data.ligand_bond_index)
        # print(data.ligand_bond_index.size())
        data.ligand_bond_type = torch.ones(
            data.ligand_bond_index.size(1), dtype=torch.long
        )
        # print(data.ligand_bond_type.size())
...
class Merge_pl(object):
    ...
    def __call__(self, data, ligand_nodes=0):
        ...
        """
        data.pocket_atom_type = torch.cat(
            [data.ligand_atom_feature, data.protein_atom_feature], dim=0
        )
        ...
        data.pocket_bond_index = torch.cat(
            [data.ligand_bond_index, protein_bond_index], dim=1
        )
        data.pocket_bond_type = torch.cat(
            [data.ligand_bond_type, data.protein_bond_type]
        )

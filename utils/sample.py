...
def construct_dataset_pocket(
    ...
    *protein_information
    ...
    # num_atom = 20
    nodesxsample_list = []
    protein_atom_feature, protein_atom_feature_full, protein_pos, protein_bond_index = (
        protein_information
    )
    num_node_frag = 0
    ...
    for n in tqdm(range(int(num_sample // batch_size))):
        ...
        if ligand_data != None:
            ...
            data = ProteinLigandData(
                ...
                ligand_num_node=num_node,
                ligand_pos=ligand_pos,
                # ligand_bond_index=ligand_bond_index, ligand_bond_type = ligand_bond_type,
                ligand_bond_index=ligand_edge_index,
                ligand_bond_type=ligand_edge_type,
            ...
            if predicted_atoms_nums is not None:
                nodesxsample = predicted_atoms_nums
            ...
            for i, n_particles in enumerate(nodesxsample):
                ...
                pos = torch.randn(n_particles, 3)
                if start_linker is not None:
                    atom_type_linker = torch.cat(
                        [start_linker["linker_atom_type"], atom_type]
                    )
                    atom_feature_linker = torch.cat(
                        [start_linker["atom_feature"], atom_feature]
                    )
                    atom_feature_full_linker = torch.cat(
                        [atom_type_linker, atom_feature_linker], dim=1
                    )
                    frag_mask = torch.cat(
                        [
                            torch.ones(num_node_frag, dtype=torch.long),
                            torch.zeros(n_particles, dtype=torch.long),
                        ]
                    )
                ...
                if start_linker is not None:
                    data = ProteinLigandData(
                        ligand_atom_feature=atom_type_linker,
                        ligand_atom_feature_full=atom_feature_full_linker,
                        ligand_num_node=num_node,
                        ligand_bond_index=adj,
                        ligand_pos=pos,
                        frag_mask=frag_mask,
                    ...
                    data = ProteinLigandData(
                        ligand_atom_feature=atom_type,
                        ligand_atom_feature_full=atom_feature_full,
                        ligand_num_node=num_node,
                        ligand_bond_index=adj,
                        ligand_pos=pos,
                        ligand_bond_type=ligand_bond_type,
...
def construct_dataset_pocket_mask(
    num_sample,
    batch_size,
    dataset_info,
    num_points=None,
    ...
    *protein_information
    ...
    for n in tqdm(range(int(num_sample // batch_size))):
        ...
        for i, n_particles in enumerate(nodesxsample):
            ...
            if start_linker is not None:
                atom_type_frag = torch.cat(
                    [start_linker["linker_atom_type"], atom_type]
                )
                atom_feature_frag = torch.cat(
                    [start_linker["atom_feature"], atom_feature]
                )
                atom_feature_full_frag = torch.cat(
                    [atom_type_frag, atom_feature_frag], dim=1
                )
                frag_mask = torch.cat(
                    [
                        torch.ones(num_node_frag, dtype=torch.long),
            ...
            if start_linker is not None:
                data = ProteinLigandData(
                    ligand_atom_feature=atom_type_frag,
...
def construct_dataset_pocket_mask_fix(
    num_sample,
    batch_size,
    dataset_info,
    num_points,
    ...
    *protein_information
    ...
    for n in tqdm(range(int(num_sample // batch_size))):
        ...
        for i, n_particles in enumerate(nodesxsample):
            ...
            atom_type_frag = torch.cat(
                [
                    torch.index_select(start_linker["linker_atom_type"], 0, frag_index),
                    torch.index_select(
                        start_linker["linker_atom_type"], 0, linker_index
                    ),
                ]
            ).float()
            ...
            atom_feature_frag = torch.cat(
                [
                    torch.index_select(start_linker["atom_feature"], 0, frag_index),
                    torch.index_select(start_linker["atom_feature"], 0, linker_index),
                ]
            ).float()
            atom_feature_full_frag = torch.cat(
                [atom_type_frag, atom_feature_frag], dim=1
            ...
            pos = torch.cat(
                [torch.index_select(start_linker["pos"], 0, frag_index), pos]
            )

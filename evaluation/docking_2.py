...
def sdf_to_pdbqt(sdf_file, pdbqt_outfile, mol_id):
    ...
    os.popen(
        f"obabel {sdf_file} -O {pdbqt_outfile} "
    ).read()
    return pdbqt_outfile
...
def calculate_qvina2_score(
    ...
    if receptor_file.suffix == ".pdb":
        ...
        receptor_pdbqt_file = Path(
            "./data/test_data/test_pdbqt_1k", receptor_file.stem + ".pdbqt"
        )
        os.popen(
            f"./miniconda/envs/adt/bin/prepare_receptor4.py -r {receptor_file} -O {receptor_pdbqt_file}"
        )
    else:
    ...
    for i, mol in enumerate(suppl):  # sdf file may contain several ligands
        ...
        if out_pdbqt_file.exists() and not out_sdf_file.exists():
            ...
            os.popen(
                f"obabel {out_pdbqt_file} -O {out_sdf_file}"
            ).read()
        if out_sdf_file.exists() and out_sdf_file.stat().st_size != 0:
            ...
            ligand_pdbqt_file = os.path.abspath("tmp/" + ligand_name + ".pdbqt")
...
if __name__ == "__main__":
    ...
    sdf_files = (
        list(args.sdf_dir.glob("*/rank1.sdf"))
